...
curl -X POST https://gateway.pipeworx.io/v1/tools/bank_of_canada_get_observations \
  ...
  -d '{"series":"FXUSDCAD","recent":10}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/bank_of_canada_get_observations`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "bank-of-canada": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-bank-of-canada"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-bank-of-canada
```
