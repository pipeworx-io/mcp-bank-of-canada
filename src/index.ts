...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Bank of Canada Valet API');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    case 'get_observations': {
      const series = validIds(reqStr(args, 'series', '"FXUSDCAD" or "FXUSDCAD,FXEURCAD"'), 'series', true);
      return bocGet(`/observations/${series}/json${rangeQuery(args)}`, 'list_series');
    }
    case 'get_group_observations': {
      const group = validIds(reqStr(args, 'group', '"FX_RATES_DAILY"'), 'group', false);
      return bocGet(`/observations/group/${group}/json${rangeQuery(args)}`, 'list_groups');
    }
    case 'series_info': {
      const series = validIds(reqStr(args, 'series', '"FXUSDCAD"'), 'series', false);
      return bocGet(`/series/${series}/json`, 'list_series');
    }
    case 'list_series': {
...
// Valet series and group names are `[A-Za-z0-9_.]+` — FXUSDCAD, V39079,
// FX_RATES_DAILY, and dotted ids like BD.CDN.AVG.YLD. A router that has
// half-learned the RBA's "F11.1 FXRUSD" phrasing hands us an id with a space;
// passed through, Valet answers 404 (or, on a bad night, 500), and a bare
// `Bank of Canada: 500 …` throw books as OUR outage. 12 external calls in 7
// days, 0 successes, all blamed on the pack (fleet #2297). Refuse the shape
// here, before any upstream call, as the caller-class mistake it is — and
// name the discovery tool so the caller can fix the id, not just retry it.
const VALET_ID_RE = /^[A-Za-z0-9_.]+$/;

function validIds(raw: string, kind: 'series' | 'group', allowList: boolean): string {
  const parts = raw.split(',').map((s) => s.trim());
  const bad = parts.filter((p) => !VALET_ID_RE.test(p));
  const discover = kind === 'series' ? 'list_series' : 'list_groups';
  const example = kind === 'series' ? 'FXUSDCAD, V39079' : 'FX_RATES_DAILY';
  if (bad.length > 0) {
    throw new Error(
      `user_error: Invalid Bank of Canada ${kind} id ${bad.map((b) => JSON.stringify(b)).join(', ')}. ` +
        `Valet ${kind} names contain only letters, digits, "_" and "." — no spaces (e.g. ${example}). ` +
        (allowList ? 'Pass several as a comma-separated list, e.g. "FXUSDCAD,FXEURCAD". ' : '') +
        `Use ${discover} to look up valid ids.`,
    );
  }
  if (!allowList && parts.length > 1) {
    throw new Error(`user_error: ${kind === 'series' ? 'series_info' : 'get_group_observations'} takes a single ${kind} id, not a list — you passed ${parts.length}. Call it once per ${kind}.`);
  }
  // Encode per id and rejoin on the literal comma Valet documents for multi-series requests.
  return parts.map((p) => encodeURIComponent(p)).join(',');
}

async function bocGet(path: string, discover?: 'list_series' | 'list_groups'): Promise<unknown> {
  const res = await pwFetch(`${BASE}${path}`, { headers: { Accept: 'application/json', 'User-Agent': UA } });
  if (!res.ok) {
    const text = (await res.text()).slice(0, 400);
    // Valet's 4xx bodies are `{"message": "Series FOO not found.", "docs": …}`.
    // Surface the message, not the JSON, and tag the class: a 404 is the caller
    // naming a series that does not exist, a 400 is a malformed argument
    // (dates, `recent`). Neither is a Pipeworx defect and neither should sit
    // on the dead-tool sweep. 5xx and everything else keep the bare status so
    // the classifier still reads them as an upstream problem.
    let message = text;
    try {
    ...
    const hint = discover ? ` Use ${discover} to look up valid ids.` : '';
    if (res.status === 404) throw new Error(`not_found: Bank of Canada Valet: ${message}${hint}`);
    if (res.status === 400) throw new Error(`user_error: Bank of Canada Valet: ${message}`);
    throw new Error(`Bank of Canada: ${res.status} ${text.slice(0, 200)}`);
  }
  return res.json();
}
...
function rangeQuery(args: Record<string, unknown>): string {
  ...
  if (typeof start === 'string' && start.trim()) {
    if (!ISO_DATE_RE.test(start.trim())) throw new Error(`user_error: start_date must be YYYY-MM-DD (got ${JSON.stringify(start)}), e.g. "2026-01-01".`);
    params.set('start_date', start.trim());
    const end = args.end_date as string | undefined;
    if (typeof end === 'string' && end.trim()) {
      if (!ISO_DATE_RE.test(end.trim())) throw new Error(`user_error: end_date must be YYYY-MM-DD (got ${JSON.stringify(end)}), e.g. "2026-03-31".`);
      params.set('end_date', end.trim());
    }
  } else {
    const recent = args.recent;
    if (recent != null && !(Number.isInteger(Number(recent)) && Number(recent) > 0)) {
      throw new Error(`user_error: recent must be a positive integer (got ${JSON.stringify(recent)}), e.g. 10.`);
    }
